...
Процедура СкопироватьФайлыВКаталог(КаталогКуда, КаталогиОткуда, МассивМасок)
 	...
 	Для Каждого КаталогПоиска Из КаталогиОткуда Цикл
		...
		Если НЕ КаталогПоискаОбъект.Существует() Тогда
			Лог.Ошибка(НСтр("hash='7C77EF5B-D752-4CA0-0C10-09530F5E025A'"));
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Маска Из МассивМасок Цикл
			...
			Для Каждого НайденныйФайл Из МассивФайлов Цикл
				...
				Исключение
					Лог.Ошибка(СтрШаблон(НСтр("hash='947CFC2F-614F-D629-95CC-C3D9052C11FF%1
|_'"), ОписаниеОшибки()));
					Продолжить;
				КонецПопытки;
			    ...
			    Если НЕ ФайлНаДиске.Существует() Тогда
					Лог.Ошибка(НСтр("hash='C46E9621-D286-6CD9-92C6-614604823667__'"));
					Продолжить;
			    КонецЕсли;
			КонецЦикла;	
...
Функция ОбеспечитьКаталог(Знач Каталог)
	...
	Если Не Файл.Существует() Тогда
		...
		Исключение
			Лог.Ошибка(СтрШаблон(НСтр("hash='C1011322-42DF-6355-595F-F7168E44EB70%1,%2
|_'"), Каталог, ИнформацияОбОшибке()));
			Возврат Ложь;
		КонецПопытки;
	ИначеЕсли Не Файл.ЭтоКаталог() Тогда 
		Лог.Ошибка(СтрШаблон(НСтр("hash='1538275F-5F82-5209-90D4-4D0D14F2FA4F%1'"), Каталог));
		Возврат Ложь;
	КонецЕсли;
