...
Процедура ПереместитьФайлыВКаталог(КаталогКуда, КаталогиОткуда, МассивМасок)
 	...
 	Для Каждого КаталогПоиска Из КаталогиОткуда Цикл
		...
		Если НЕ КаталогПоискаОбъект.Существует() Тогда
			Лог.Ошибка(НСтр("hash='82D18841-F445-5571-1263-3C9EE09FE398'"));
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Маска Из МассивМасок Цикл
			...
			Для Каждого НайденныйФайл Из МассивФайлов Цикл
				...
				Исключение
					Лог.Ошибка(СтрШаблон(НСтр("hash='B99A46F7-788A-CF51-1FA8-8A3073E0EE3E%1
|_'"), ОписаниеОшибки()));
					Продолжить;
				КонецПопытки;
			    ...
			    Если НЕ ФайлНаДиске.Существует() Тогда
					Лог.Ошибка(НСтр("hash='04955DC6-DAC1-A408-88DA-A6B370F12574__'"));
					Продолжить;
			    КонецЕсли;
			КонецЦикла;	
...
Функция ОбеспечитьКаталог(Знач Каталог)
	...
	Если Не Файл.Существует() Тогда
		...
		Исключение
			Лог.Ошибка(СтрШаблон(НСтр("hash='5D3CAA7D-644B-28DA-A330-0403D1891EB9%1,%2
|_'"), Каталог, ИнформацияОбОшибке()));
			Возврат Ложь;
		КонецПопытки;
	ИначеЕсли Не Файл.ЭтоКаталог() Тогда 
		Лог.Ошибка(СтрШаблон(НСтр("hash='DCD0CFE1-4598-084C-CE37-7F3FC41D8B5A%1'"), Каталог));
		Возврат Ложь;
	КонецЕсли;
