...
Процедура ПереместитьФайлыВКаталог(КаталогКуда, КаталогиОткуда, МассивМасок)
 	...
 	Для Каждого КаталогПоиска Из КаталогиОткуда Цикл
		...
		Если НЕ КаталогПоискаОбъект.Существует() Тогда
			Лог.Ошибка(НСтр("hash='03B07F7B-47B5-F40E-E312-2FA0DDE681E7'"));
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Маска Из МассивМасок Цикл
			...
			Для Каждого НайденныйФайл Из МассивФайлов Цикл
				...
				Исключение
					Лог.Ошибка(СтрШаблон(НСтр("hash='DFF4414E-BEDF-F179-9FAC-CB0524B3A7F2%1
|_'"), ОписаниеОшибки()));
					Продолжить;
				КонецПопытки;
			    ...
			    Если НЕ ФайлНаДиске.Существует() Тогда
					Лог.Ошибка(НСтр("hash='5361FC52-358D-5063-3BDD-D52B1CC2D905__'"));
					Продолжить;
			    КонецЕсли;
			КонецЦикла;	
...
Функция ОбеспечитьКаталог(Знач Каталог)
	...
	Если Не Файл.Существует() Тогда
		...
		Исключение
			Лог.Ошибка(СтрШаблон(НСтр("hash='DF26A67E-46B5-E9DF-F7AF-FDEF1201EA42%1,%2
|_'"), Каталог, ИнформацияОбОшибке()));
			Возврат Ложь;
		КонецПопытки;
	ИначеЕсли Не Файл.ЭтоКаталог() Тогда 
		Лог.Ошибка(СтрШаблон(НСтр("hash='E58D4BAF-B606-9EA0-05CA-AD37998C24AE%1'"), Каталог));
		Возврат Ложь;
	КонецЕсли;
